...
async function filter() {
  try {
    ...
    const response = await fetch("../../BLL/ProductBLL.php", {
      ...
      body:
        "function=" +
        encodeURIComponent("filter_product_by_atributive") +
        "&filter=" +
        encodeURIComponent(stringFilter) +
        "&page=" +
        encodeURIComponent("1") +
        "&limit=" +
        encodeURIComponent("12"),
    });
...
async function load_value_targetGender_filter() {
  try {
    ...
    const response = await fetch("../../BLL/ProductBLL.php", {
      ...
      body: "function=" + encodeURIComponent("getArrObj_targetGender"),
    });
    let data = await response.json();
...
async function load_value_color_filter() {
  try {
    ...
    const response = await fetch("../../BLL/ProductBLL.php", {
      ...
      body: "function=" + encodeURIComponent("getArrObj_color"),
    });
    let data = await response.json();
    // console.log(dataTargetGender);
    if (data != null) {
...
async function load_value_bagMaterial_filter() {
  try {
    ...
    const response = await fetch("../../BLL/ProductBLL.php", {
      ...
      body: "function=" + encodeURIComponent("getArrObj_bagMaterial"),
    });
    let data = await response.json();
    // console.log(dataTargetGender);
    if (data != null) {
...
async function load_value_shirtMaterial_filter() {
  try {
    ...
    const response = await fetch("../../BLL/ProductBLL.php", {
      ...
      body: "function=" + encodeURIComponent("getArrObj_shirtMaterial"),
    });
    let data = await response.json();
    // console.log(dataTargetGender);
    if (data != null) {
...
async function load_value_shirtStyle_filter() {
  try {
    ...
    const response = await fetch("../../BLL/ProductBLL.php", {
      ...
      body: "function=" + encodeURIComponent("getArrObj_shirtStyle"),
    });
    let data = await response.json();
    // console.log(dataTargetGender);
    if (data != null) {
...
async function load_value_priceMax_filter() {
  try {
    ...
    const response = await fetch("../../BLL/ProductBLL.php", {
      ...
      body: "function=" + encodeURIComponent("getArrObj_price"),
    });
    let data = await response.json();
    // console.log(dataTargetGender);
    if (data != null) {
      ...
      if (data.length > 0) {
        ...
        for (let i of data) {
          let string = `
                        <li>
                            <input style="width: 200px;" type="range" min="0" step="1" max="${Math.ceil(
                              i
                            )}" value="${Math.ceil(i)}">
                        </li>
                        <li>
...
function showResultFilter(page, lengthData) {
  ...
  let containerListProduct = document.querySelectorAll(
    ".container-path .product-list .product-container"
  );
  let lengthItem = containerListProduct.length;
  let start = (page - 1) * 12 + 1;
  let string = `Show ${start}–${
    start + lengthItem - 1
  } of ${lengthData} result`;
...
async function Pagination_3(page, limit, lengthProduct) {
  // làm hình tượng load
  let containerListProduct = document.querySelector(
    ".container-path .product-list"
  );
...
async function listPage_3(lengthProduct, limit, thisPage) {
  ...
  for (let i = 1; i <= count; i++) {
    ...
    newPage.setAttribute(
      "onclick",
      "changePage_3(" + i + "," + limit + "," + lengthProduct + ")"
    );
    document.querySelector(".container-path .list-page").appendChild(newPage);
  }
}
            ...
            <div class="product-container">
                ...
                <div class="image">
                    ...
                    <div class="action-custom">
                        <!--cart-->
                        <div class="add-to-cart">
                            <a href="./Cart.php?code=${mahoaProductCode}&type=${mahoaType}&quantityBuy=${btoa(
            "1"
          )}&addCart=${btoa(
            "true"
          )}&sizeCode=${sizeCodeValue}" class="" title="add cart">
                                <i class="fa fa-shopping-bag" aria-hidden="true"></i>
                            </a>
                        </div>
                        <!--read-more-->
                ...
                <div class="price">
                    <div itemprop="offers" class="pricelist"></div>
                    <ins class="giamoi"><span class="price-sale">$${
                      i.price
                    }</span> $${promotionPrice}</ins>
                </div>
            </div>
...
function action() {
  ...
  document
    .querySelector("#filter-price input")
...
// chi thực hien cac hanh dong khi load xong
